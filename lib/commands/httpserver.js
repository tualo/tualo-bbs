  ...
  module.exports = HttpServer = (function(superClass) {
    ...
    HttpServer.prototype.controller = function(sequenceFN, onClosed, onDone, onError, runseq) {
      ...
      if (me.queryIsRunning) {
        ...
        this.ctrl.setIP(args.machine_ip, args.machine_port);
        this.ctrl.on('error', function(msg) {
          me.queryIsRunning = false;
          if (typeof onError === 'function') {
            return onError(msg);
          }
        });
        this.ctrl.once('closed', function(msg) {
          if (process.env.DEBUG_BBS_HTTPSERVER === '1') {
            console.log('controller', sequenceFN, 'ctrl close', msg);
          }
          return onClosed(msg);
        });
        this.ctrl.once('ready', function() {
          var seq;
          me.queryIsRunning = false;
          ...
          seq.once('statuslight', function(msg) {
            if (process.env.DEBUG_BBS_HTTPSERVER === '1') {
          ...
          seq.once('startjob', function(msg) {
            if (process.env.DEBUG_BBS_HTTPSERVER === '1') {
